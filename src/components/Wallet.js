...
function Wallet({currentUser, currentWallet, currentUser:{bills}}) {
  ...
  useEffect(() => {
    fetch(`https://spendsy-api.herokuapp.com/userwallet/${currentWallet.id}`)
    .then(resp => resp.json())
    .then(data => setWalletBills(data.bills))

    fetch(`https://spendsy-api.herokuapp.com/total/${currentWallet.id}`)
    .then(resp => resp.json())
...
const updateWalletInfo = (id) => {
  ...
  fetch (`https://spendsy-api.herokuapp.com/wallets/${id}`, {
    method: 'PATCH',
    headers: {
...
// Get all the bills for the wallet (after deleting or editing a bill)
  const getBills = (currentWallet) => {
    fetch(`https://spendsy-api.herokuapp.com/userwallet/${currentWallet}`)
    .then(resp => resp.json())
    .then(data => setWalletBills(data.bills))
  }
...
//  Delete a bill
  function handleBillDelete(id, currentWallet){
     if(window.confirm("Are you sure you want to delete this bill?")){
        fetch(`https://spendsy-api.herokuapp.com/bills/${id}`,{
         method: "DELETE",  
        })
        .then(() => {
